...
It will ask for a password, use something secure. You can just hit enter through all the fields. I used the user "SecretUserName" but you can use anything
...
```
# usermod -aG sudo SecretUserName
```
...
```
$ ssh SecretUserName@YOUR_SERVER_IP
```
...
```
# git clone https://github.com/yourgithubname/Your_Project_Name.git
```
...
Create a file called **.env** in src folder on your server and add the following  ## .env for python decouble library to secure creadentails
See the Example .env File for more info
- SECRET_KEY
- DATABASES_Credentials
- DEBUG  ## Always make it False
- EMAIL_Credentials
- ANY OTHER CREDENTIALS YOU WANT TO PROTECT
...
### Go to settings/production.py on the server and change "ALLOWED_HOSTS" to include the domain
